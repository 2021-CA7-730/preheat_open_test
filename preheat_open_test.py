...
# %%
if do_plot:
    plt.plot(pd.to_datetime(DC01.data.index), DC01.data.values, label=DC01.name)
    plt.plot(pd.to_datetime(DC02.data.index), DC02.data.values, label=DC02.name)
    plt.plot(pd.to_datetime(DC03.data.index), DC03.data.values, label=DC03.name)
    if res == "raw":
