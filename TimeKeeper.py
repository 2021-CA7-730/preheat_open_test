...
Edited on Fri Oct 1 12:49:23 2021
@author: japem
"""
...
class TimeKeeper():
    ...
    def create_schedule_dt(self, time_start, time_end, step_size):
    ...
    def get_local_time_str():
        local_time = datetime.datetime.now(
            datetime.timezone(datetime.timedelta(0))).astimezone()
    ...
    def __dt2str(self, dt, accuracy='iso'):
        ...
        if accuracy == 'sec':
            ...
            raise(ValueError(
                'The keyword {key} does not exist'.format(key=accuracy)))
    ...
    def create_std_plan(self, event_times, values):
        event_times_off = event_times + \
            [event_times[-1] + datetime.timedelta(seconds=3*60)]
